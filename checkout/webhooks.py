...
def webhook(request):
    ...
    print(f"[WEBHOOK DEBUG] Secret: {wh_secret[:5]}... "
          f"(length: {len(wh_secret)})")
    # Get the webhook data and verify its signature
    payload = request.body
    sig_header = request.META.get("HTTP_STRIPE_SIGNATURE")
    print(f"[WEBHOOK DEBUG] Payload length: {len(payload)}")
    print(f"[WEBHOOK DEBUG] Signature header present: "
          f"{sig_header is not None}")
    ...
    try:
        ...
        print(f"[WEBHOOK DEBUG] ✅ Verification successful for event: "
              f"{event['type']}")
