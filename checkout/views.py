...
def cache_checkout_data(request):
    try:
        ...
        # Use the dictionary unpacking approach for metadata
        metadata = {
            'username': request.user.username if request.user.is_authenticated else 'AnonymousUser',
        }
        
        # Only add bag to metadata if it exists in session
        if 'bag' in request.session:
            metadata['bag'] = json.dumps(request.session.get('bag', {}))
        
        # Add save_info only if it was passed
...
def checkout_success(request, order_number):
    ...
    try:
        ...
        messages.error(request, f'Order confirmed, but we had an issue sending your confirmation email. Error: {e}')
