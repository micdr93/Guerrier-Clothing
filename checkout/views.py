    ...
    try:
        intent = stripe.PaymentIntent.create(
            ...
            metadata={
                ...
                "save_info": "true" if request.session.get("save_info") else "false"
            }
        )
...
def cache_checkout_data(request):
    try:
        client_secret = request.POST.get("client_secret", "").strip().strip('"\'')
        pid = client_secret.split("_secret")[0] if "_secret" in client_secret else client_secret
        pid = pid.strip().strip('"\'')

        if not pid or not pid.startswith("pi_"):
            raise ValueError(f"Invalid payment intent ID: {pid}")
        ...
        metadata["save_info"] = save_info == "true"
        stripe.PaymentIntent.modify(pid, metadata=metadata)
        return HttpResponse(status=200)
    except Exception as e:
        print(f"STRIPE ERROR (Checkout Data Caching): {str(e)}")
...
def checkout_success(request, order_number):
    ...
    if request.user.is_authenticated:
        try:
            ...
            messages.error(request, "Order confirmed, but we had an issue sending your confirmation email. Error: " + str(e))
    try:
        send_confirmation_email(order)
    except Exception as e:
        messages.error(request, "Order confirmed, but we had an issue sending your confirmation email. Error: " + str(e))
    messages.success(request, "Order successfully processed! Your order number is " + order_number + ". A confirmation email will be sent to " + order.email + ".")
    if "bag" in request.session:
