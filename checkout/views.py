...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        # Create a payment intent with the correct amount and currency
        try:
            ...
            # Clear client secret if present in strings
...
def order_detail(request, order_number):
    """
    Display an individual order's details
    """
    order = get_object_or_404(Order, order_number=order_number)
    ...
    if request.user.is_authenticated:
        profile = UserProfile.objects.get(user=request.user)
        # Only allow the user who placed the order or admins to view it
        if order.user_profile != profile and not request.user.is_superuser:
