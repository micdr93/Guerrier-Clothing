...
class StripeWH_Handler:
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        try:
            ...
            logger.error(f"Error retrieving Stripe charge: {e}")
            return HttpResponse(
        ...
        if order_exists:
            ...
            logger.info(f"Verified existing order {order.id} for payment intent {pid}")
            return HttpResponse(
            ...
            self._send_confirmation_email(order)
            logger.info(f"Created order {order.id} from webhook payment intent {pid}")
            return HttpResponse(
    ...
    def handle_payment_intent_payment_failed(self, event):
        ...
        logger.info(f"Payment failed webhook received: {event['type']}")
        return HttpResponse(
